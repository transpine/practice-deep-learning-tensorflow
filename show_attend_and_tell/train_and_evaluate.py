...
# 이 부분이 없으면 tensorflow.reshape을 못찾는등의 문제가 생긴다.
...
from data_util import prepare_image_and_caption_data, cache_bottlenecks, maybe_download_and_extract
from utils import load_image, calc_max_length, plot_attention

flags.DEFINE_bool('do_caching', True, 'Do bottleneck caching')
FLAGS = flags.FLAGS
...
def loss_function(real, pred):  
  # 문장이 짧은경우 loss를 계산하지 않도록. mask로 처리
...
def train_step(img_tensor, target, tokenizer, encoder, decoder):
  ...
  dec_input = tensorflow.expand_dims([tokenizer.word_index['<start>']] * target.shape[0], 1)
...
def evaluate(image, max_length, attention_features_shape, encoder, decoder, image_features_extract_model, tokenizer):
  ...
  # img_tensor_val = np.reshape(img_tensor_val, (img_tensor_val.shape[0], -1, img_tensor_val.shape[3]))
  ...
  for i in range(max_length):
    ...
    # attention_plot[i] = np.reshape(attention_weights, (-1,)).numpy()

    predicted_id = tensorflow.random.categorical(predictions, 1)[0][0].numpy()
...
def main(_):
  ...
  # image_features_extract_model을 tensorflow.keras.Model을 이용해서 선언합니다.
  ...
  tokenizer = tensorflow.keras.preprocessing.text.Tokenizer(num_words=top_k,
                                                    oov_token="<unk>",
                                                    filters='!"#$%&()*+.,-/:;=?@[\]^_`{|}~ ')
  ...
  cap_vector = tensorflow.keras.preprocessing.sequence.pad_sequences(train_seqs, padding='post')
  ...
  dataset = tensorflow.data.Dataset.from_tensor_slices((img_name_train, cap_train))
  # numpy 파일들을 병렬적(parallel)으로 불러옵니다.
  dataset = dataset.map(lambda item1, item2: tensorflow.numpy_function(
            map_func, [item1, item2], [tensorflow.float32, tensorflow.int32]),
            num_parallel_calls=tensorflow.data.experimental.AUTOTUNE)

  # tensorflow.data API를 이용해서 데이터를 섞고(shuffle) batch 개수(=64)로 묶습니다.
  ...
  ckpt = tensorflow.train.Checkpoint(encoder=encoder,
                             decoder=decoder,
                             optimizer = optimizer)
  ...
  image_path = tensorflow.keras.utils.get_file('image' + image_extension, origin=image_url)
