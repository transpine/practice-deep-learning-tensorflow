...
class BatchDataset:
  ...
  def _read_images(self):
    ...
    # self.images = np.array([self._transform(filename['image']) for filename in self.files], dtype=object)

    # for filename in self.files:
    #   self.images.append(self._transform(filename['image']))
    #   print(f"filename: {filename['image']}")
    ...
    # for filename in self.files:
    #   self.annotations.append(np.expand_dims(self._transform(filename['annotation']), axis=3))
    #   print(f"annotation: {filename['annotation']}")
  ...
  def _transform(self, filename):
    ...
    if self.image_options.get("resize", False) and self.image_options["resize"]:
      ...
      resize_image = Image.fromarray(image).resize(size=(resize_size, resize_size))
    ...
    # print(f'## SHAPE:{np.array(resize_image).shape}')
    # print(f'## squeezed SHAPE:{(resize_image[:,:, 0]).shape}')

    # if len(resize_image.shape) > 2:
    #   return np.array(resize_image[:, :, 0])
    # else:
